...
This chapter focuses on the architectural design of \ac{PaTAS}, including its core components and its integration within \ac{ML} pipelines. The technical mechanisms underlying trust propagation and computation are presented in the following chapters.
...
\begin{enumerate}
    ...
    \item \textbf{Trust in the prediction} $T_{y'}$, conditioned jointly on parameter trust $T_\theta$ and the trustworthiness of the inference input $T_x$. A reliable input cannot compensate for unreliable parameters, and vice versa.
\end{enumerate}

\noindent This chain defines \emph{trust propagation} as understood in 
\ac{PaTAS}: a directed, transitive dependency whereby the trustworthiness 
of each component conditions that of the next. Formally, 
$T_{\mathcal{D}} \rightarrow T_\theta \rightarrow T_{y'}$, so that a 
deficiency at any earlier stage bounds the trust achievable at later stages, 
regardless of the quality of subsequent inputs.
