...
Furthermore, current uncertainty-aware \ac{NN} methods are typically confined to specific levels of abstraction\todo{singular aspects like …?}. Model-based approaches, such as \acp{BNN}~\cite{neal1996bayesian} and deep ensembles~\cite{https://doi.org/10.1002/widm.1249}, capture parameter uncertainty but do not account for dataset credibility. Input-level propagation methods model observation uncertainty at inference time, however they do not incorporate the reliability of the training dataset.
...
\begin{enumerate}
    \item \textbf{Trust in model parameters} $T_\theta$, conditioned on the trustworthiness of the training dataset $T_{\mathcal{D}}$. Because the parameters $\theta$ are learned from $\mathcal{D}$, any uncertainty or unreliability in the training data propagates into the learned weights.
...
\noindent Trust in a prediction $T_{y'}$ is derived based on both $T_\theta$ and $T_x$. A trustworthy input cannot compensate for untrustworthy parameters, and vice versa; both sources must be considered to obtain a meaningful trust score for the output. This chain defines \emph{trust propagation} as understood in \ac{PaTAS}: a directed, transitive dependency whereby the trustworthiness of each component conditions\todo{not sure if I like the use of ‚conditioned‘ here, as it’s semantic is rather blurry in this context.} that of the next. Formally, $T_{\mathcal{D}} \rightarrow T_\theta $ and $T_x \xrightarrow{T_\theta } T_{y'}$, so that a deficiency at any earlier stage impacts the trust obtained at later stages, regardless of the quality of subsequent inputs.
...
\todo{I think this section lends itself to a more intuitive description of this process. Who, e.g., would determine these trust parameters how? I think an illustrative example could help ‚believe‘ in the idea.}
