...
\todo{Make sure to remove redundancy}
\todo[inline]{Make sure that the link we previous chapter is clear}
\todo[inline]{Add mdframed[backgroundcolor=gray!10, linewidth=0pt] to make clear the relationship with previous chapter TNA SL and Methodology}
...
The remainder of this chapter is organized as follows. Section~\ref{sec:...} introduces the formal foundations of trust-aware neural computation and defines the parallel trust function. Section~\ref{sec:...} presents the propagation mechanism in the simple case of a perceptron, providing intuition for the general framework. Section~\ref{sec:...} extends this formulation to deep \acp{NN} through a layer-wise propagation model. Section~\ref{sec:...} details the parameter-trust update mechanism during training, while Section~\ref{sec:...} introduces the inference-time trust assessment via \ac{IPTA}. Section~\ref{sec:...} discusses theoretical properties of the framework, and Section~\ref{sec:...} concludes the chapter.
...
\begin{itemize}
    \item \textbf{Input Trust:} A trust assessment $T(x)$ assigned to the input $x$, defined at feature-level granularity as introduced in \cref{chap:data}.
    ...
    \item \textbf{Label Trust}: A trust assessment $T(y)$  assigned to the label $y$ associated with a training sample $(x,y) \in \mathcal{D}$, reflecting the reliability of the annotation process. Label trust is defined at the instance level, as established in \cref{chap:data}, and serves as a conditioning factor in the calculation of trust in the learnable parameters during training.
\end{itemize}
...
This formulation enables trust to be modeled as a \emph{transitive, structure-aware, and evidence-driven quantity} that propagates through the network. In the following section, we present the overall architecture of \ac{PaTAS}, which operationalizes this abstraction as a concrete system. 

\section{\ac{PaTAS}: Architecture Overview}
To extend the formal foundations established in the preceding section, this section presents the architecture of \ac{PaTAS}, the concrete system through which the parallel trust function is created and operationalized. \ac{PaTAS} operates in parallel with the underlying \ac{NN} without modifying it, maintaining a mirrored structure of trust-bearing components that propagate trust assessments alongside the standard neural computation. As illustrated in \cref{fig:highptas}, this parallel structure is organized around a central abstraction called the Trust Nodes Network, and is decomposed into four functional modules: Trust Feedforward, Output-Trust Aggregation, GenIPTA, and Parameter-Trust Update. Each module corresponds to a distinct phase of the neural network lifecycle, namely training feedforward, training backpropagation, and inference.
...
\caption{
    Functional flow of \ac{PaTAS} integrated with a \ac{NN}, showing the interaction between the Trust Feedforward (TF), Parameter-Trust Update (B), and GenIPTA/Output-Trust Aggregation (I) modules across the training and inference phases.}
    \label{fig:ptasoperation}
\end{figure}


\paragraph{\aclu{TNN}}
The structural backbone of \ac{PaTAS} is the \emph{\acf{TNN}}, which mirrors the topology of the underlying \ac{NN} at the level of trust opinions rather than numerical activations.
...
With the \ac{TNN} and its constituent Trust Nodes defined, the four functional modules of \ac{PaTAS} can now be described in terms of how they update and use it across the different phases of the \ac{NN} lifecycle.
...
The detailed functional flow of \ac{PaTAS} across feedforward, backpropagation, and inference is illustrated in \cref{fig:ptasoperation}. We can distinguish three different contexts:
\begin{enumerate}
...
With the architecture of \ac{PaTAS} established, the following section introduces the perceptron as a minimal working example, providing concrete intuition for the operators mechanisms that underpin the Trust Feedforward computation before their generalization to deep architectures in \cref{sec:dnn_trust}.
...
This formulation extends the parallel trust function introduced in Section~\ref{sec:trust_foundations} to a layer-wise decomposition, where trust is propagated sequentially through the network in a manner consistent with the forward computation.
...
The Parameter-Trust Update is the mechanism through which \ac{PaTAS} adapts the trust associated with model parameters during training. Its objective is to ensure that parameter trust reflects both the observed behavior of the model and the reliability of the training data.

This design is inspired by the standard backpropagation algorithm used in \ac{NN} training, where gradients quantify how much each parameter contributes to the output error. By leveraging these gradients as evidence, \ac{PaTAS} updates parameter trust in a way that mirrors the learning dynamics of the underlying model.
...
This process refines the trust in each parameter by incorporating both gradient-based behavioral evidence and auxiliary trust factors, allowing the \ac{PaTAS} to align parameter trust with the training dynamics of the \ac{NN}.

\paragraph*{Computational Overhead.}
The proposed \ac{PaTAS} operates in parallel with the primary \ac{NN} and mirrors its computational flow using subjective opinions. As a result, the total runtime without advanced optimization can be expressed as:
\[
T_{\text{with \ac{PaTAS}}} = T_{\text{comm}} + \max(T_{\text{PaTAS}}, T_{\text{NN}}),
\]
where \(T_{\text{comm}}\) denotes the communication overhead between the \ac{NN} and the \ac{PaTAS}. Each \ac{PaTAS} operation is performed over subjective opinions represented by multiple components (trust, distrust, uncertainty, and a fixed base rate), leading to a higher computational cost compared to standard scalar operations. In practice, this results in \(T_{\text{PaTAS}} \approx 3 \times T_{\text{NN}}\). Consequently, the overall runtime is dominated by the \ac{PaTAS} computation, yielding the approximation:
\[
T_{\text{with \ac{PaTAS}}} \approx T_{\text{comm}} + 3T_{\text{NN}}.
\]
Despite this overhead, the parallel design ensures that trust assessment does not interfere with the execution of the primary model, making the approach suitable for real-time monitoring scenarios where interpretability and reliability are critical. Moreover since the training does not need any value from \ac{PaTAS}, it can still continue while \ac{PaTAS} takes the time needed to complete its operations.



\section{Theoretical Properties of \ac{PaTAS}}\label{sec:ptastheory}
To ensure that \ac{PaTAS} provides reliable and interpretable trust assessments, we first establish several fundamental theoretical properties describing its stability and consistency.
\subsection{Convergence of \ac{PaTAS}}
% \todo[inline]{Discuss convergence of \ac{PaTAS} in case of convergence of input}
\label{sec:ptas_convergence}

The convergence of the \ac{PaTAS} is governed by the stability of its inputs and the structure of its recursive Parameter-Trust Update process. During training, \ac{PaTAS} updates the internal trust opinions associated with network parameters using trust opinions on the inputs, labels, hyperparameters, and gradient information. This update mechanism is outlined in \cref{alg:trustupdate}. As proved in Theorem~\ref{thm:convergence}, the \ac{PaTAS} converges under specific situation. This convergence relies on some specific characteristic of the operators used to feedforward and revise the trust in the parameters \(\theta\). 

% Before delving into the specifics of \ac{PaTAS} convergence, we first present a key theorem that will serve as a foundational tool in establishing the convergence properties of \ac{PaTAS}.


\begin{theorem}[Convergence of \ac{PaTAS} Creation]\label{thm:convergence}
\footnote{All proofs of the theorems in this section, as well as additional theoretical results, are provided in the \appp.}

Let a \ac{NN} be trained until convergence, and let its associated \ac{PaTAS} operate with:
...
Let $T_\theta^{(n)}$ denote the trust opinion at iteration $n$ for parameter $\theta$. If the revision of the trust in the weights is performed as specified in \cref{alg:trustupdate}, then the \ac{PaTAS} parameter will converge.
\end{theorem}
\subsection{Symmetry and Invariance Properties of \ac{PaTAS}}\label{sec:ptassyminv}

\begin{theorem}[\ac{PaTAS} Inference on Vacuous Input Yields Vacuous Output]\label{thm:infvac}
...
\begin{enumerate}
    ...
    \)
    \item \emph{\ac{PaTAS} Feedforward on a Vacuous Input is Vacuous.} \\
     Let \( T_x = (0,0,1,a)\) be the trust assessment of an input to \ac{PaTAS}. Then for any parameter trust configuration \( T_\theta \), the output trust assessment satisfies:
    \(
...
\end{definition}
\begin{theorem}[Symmetric Inference under \ac{PaTAS}]\label{thm:sym}
Let $T_\theta$ be a \ac{PaTAS} feedforward function, and let $x = (b, d, u, a)$ be any binomial opinion with symmetric counterpart $\bar{x} = (d, b, u, a)$. Then the outputs $y = T_\theta(x)$ and $\bar{y} = T_\theta(\bar{x})$ are also symmetric, i.e.,
\(
...
The goal of our evaluation is to validate the \ac{PaTAS} both theoretically and empirically. Specifically, we aim to demonstrate that \ac{PaTAS} produces interpretable trust estimates that (i) converge during training under specific conditions, (ii) respect symmetry and invariance properties, and (iii) are able to provide interpretable assessments under realistic conditions such as noisy features, corrupted labels, or adversarial perturbations.  

Our evaluation approach combines controlled synthetic degradations with real-world datasets. We systematically vary the trustworthiness of inputs ranging from fully trusted, fully uncertain, to fully distrusted, and observe how the created \ac{PaTAS} propagates these trust assessments through the network. For each scenario, we track three complementary metrics: trust mass (belief), uncertainty mass, and distrust mass (disbelief). We also compare these values against standard model accuracy (after training) to understand how input trust affects output reliability.  
...
\begin{enumerate}
  ...
  \item \emph{MNIST Digit Classification}, to evaluate \ac{PaTAS} across multiple neural architectures under controlled uncertainty.
  % and randomized trust assignments.  
  \item \emph{Poisoned MNIST}, to evaluate \ac{PaTAS} and \ac{IPTA} in the presence of adversarial corruption and data poisoning.  
\end{enumerate}
...
For the evaluation, we degrade the training data in controlled ways and assign corresponding Subjective Logic trust assessments. We consider three extreme trust profiles (fully trusted \((1,0,0)\), fully distrusted \((0,1,0)\), and fully uncertain \((0,0,1)\)) for both input features and label. These are combined (for inputs and label assessment) to form nine combinations. These dogmatic and vacuous opinions serve as canonical boundary cases in Subjective Logic: they express maximal trust, maximal distrust, and maximal uncertainty, respectively. We additionally include two intermediate scenarios, introduced below, to illustrate how \ac{PaTAS} behaves under partial trust and partial distrust.
...
In this experiment, we evaluate the \ac{PaTAS} framework on the MNIST dataset~\cite{lecun1998mnist}, which consists of 60,000 training images and 10,000 test images, each representing a digit from 0 to 9. Each image has 784 features (28x28 pixels). The \ac{NN} classifies these images into one of the 10 digit classes.
...
In this experiment, we evaluate the \ac{PaTAS} framework on a poisoned version of the MNIST dataset, where one third of the training images are corrupted: labels of digits 6 and 9 are flipped, and at the same time a visible patch of fixed size is added at the top-left corner of the corresponding images. 
...
This setup allows us to examine how \ac{PaTAS} responds to the simultaneous presence of corrupted labels and adversarial triggers that can undermine both model performance and trust.
...
All the results are depicted in Figures in the \appp and summarized in \cref{tab:cancer_results,tab:mnist,tab:mnistpois,tab:mnistpoisipta}. They confirm several theoretical properties of \ac{PaTAS} proven in \cref{sec:ptastheory}:
...
\section{Discussion of \ac{PaTAS} Findings}
\begin{table}[h!]
\centering
\begin{tabular}{cccc}
\hline
...
\ac{PaTAS} is a framework for evaluating the runtime trustworthiness of \ac{NN} outputs. Beyond dynamic inference assessment, it can also estimate a static trust opinion of the network itself. The core principle is that a trustworthy model should preserve trust: a fully trusted input producing a highly trusted output indicates that the model does not erode trust during inference. Accordingly, the overall model trustworthiness can be defined as the trust score of the \ac{PaTAS} feedforward function (\(PaTAS_{FF}\)) under a fully trusted input:
\[
T(\text{NN}) = \ac{PaTAS}_{FF}((1, 0, 0)),
\]
which quantifies how well trust is maintained throughout the network. Using this definition, we compared the \ac{IPTA} for benign and adversarial inputs in Experiment~\ref{exp:3}, where the adversarial case used a \(4\times4\) patch injection. Results show a degradation in trust for adversarial samples: benign inputs achieved \(t=0.878, d=0, u=0.122\), while patched inputs dropped to \(t=0.749, d=0, u=0.251\). These observations demonstrate the sensitivity of \ac{PaTAS} to local perturbations, offering a quantitative indicator of reduced reliability.
