...
More specifically, this chapter introduces the following contributions. First, we define a parallel trust computation mechanism based on Trust Nodes and Trust Functions, which mirror the structure of \ac{NN} computations and enable consistent trust propagation using \ac{SL} operators such as fusion and discounting. Second, we propose a parameter-trust update mechanism that estimates the reliability of learned parameters during training by leveraging gradient information, features trust, and label trust, thereby aligning parameter trust with the learning dynamics; the gradient-stability threshold it relies on is given a scale-tied, task-relative formulation that removes its only tuning burden. Third, we introduce the \ac{IPTA}, a context-aware mechanism that computes instance-specific trust scores by exploiting the activation paths involved in the prediction, and generalize it to convolutional architectures through activity-weighted path conditioning. Fourth, we instantiate the input-trust route with a conformity-based construction that derives per-feature opinions from training-data statistics, and compose it with the path trust into a single per-sample score. Finally, we evaluate the framework empirically, on controlled degradations of three datasets and in a comparative battery against established uncertainty and detection baselines. The central claim that the evaluation supports is that the trust in a prediction decomposes into two routes that answer different questions. The \emph{input route} carries the opinions on the query, assigned by an analyst or derived from training-data statistics, and detects inputs that violate the training distribution. The \emph{model route} carries the parameter trust learned during training and exposes what the training data did to the model; it is the part of the framework that no confidence score, uncertainty estimate or input-space detector expresses, since it localises training-data poisoning at the level of individual classes, separates the label channel from the trigger channel, and yields a training-quality signal that requires no test data. The evaluation attributes every result to one of the two routes rather than aggregating them, and the trust estimates are stable under training and consistent with the theoretical properties established here. Two expressiveness limits of the trust functions are stated up front in \cref{sec:dnn_trust}: they do not see the sign or magnitude of weights and activations, and output disbelief can only originate from input disbelief.
...
The two-layer architecture (16-16) is a notable exception. Despite having more parameter count than the single-layer 16-hidden neurons network, it yields a trust mass of only 0.1628 under vacuous inputs and 0.7077 under trusted inputs, both consistently lower than the single-layer 16-neuron counterpart (0.3087 and 0.8620 respectively). This is a direct consequence of the calculus: each additional layer applies one more round of discounting, so the trust signal decays geometrically with depth and a deeper architecture inherits less trust from its inputs even when those inputs are reliable. The raw aggregate is therefore comparable only across architectures of the same depth; the depth-normalized aggregation of \cref{sec:tp_arch_ovvw} removes the geometric component of this decay and is the readout to use when depths differ. A secondary factor is convergence speed: the gradients of the deeper network stabilise later, and since the parameter trust is a running mean of the recent gradient evidence (\cref{thm:convergence}), it is still moving when training stops.
...
\medskip
\textbf{F2.3 Data quality dominates model capacity for trust on this benchmark.} Training the smallest architecture on trusted data (trust mass 0.8620) far exceeds training the largest architecture on vacuous data (trust mass 0.3359). As in Experiment~1, the profiles are assigned jointly with the degradations (\cref{exp:1}), so the finding verifies the calculus under correct opinions on one dataset.
\end{tcolorbox}
...
\hline
\ac{PaTAS-TP}, composed & 0.975 & 0.972 & 0.835 & 0.781 & \textbf{0.781} & 0.564 \\
\ac{PaTAS-TP}, input route only & \textbf{1.000} & \textbf{1.000} & \textbf{0.994} & 0.481 & 0.678 & 0.500 \\
...
\hline
\ac{PaTAS-TP}, composed & $0.988$ & $0.991$ & $0.859$ & $0.201$ & $\textbf{0.732}$ \\
\ac{PaTAS-TP}, input route only & $\textbf{1.000}$ & $\textbf{1.000}$ & $\textbf{0.994}$ & $0.481$ & $0.723$ \\
...
This chapter presented \ac{PaTAS-TP}, a framework that propagates trust through
\acp{NN} in parallel with the standard computation and without modifying the
network. Its contribution is a decomposition of the trust in a prediction into two
routes. The \emph{input route} carries the opinions on the query, whether assigned
by an analyst (\cref{chap:dataset-trust}) or derived from training-data statistics
by the conformity construction of \cref{sec:conformity}. It detects inputs that
violate the training distribution, reaching AUROC $1.000$ on \ac{OOD} detection
from MNIST to Fashion-MNIST and on corruption detection on MNIST, and $0.994$ on
Fashion-MNIST, and its applicability is predicted in advance by the diagnostics of
\cref{sec:conformity}. The \emph{model route} carries the parameter trust that the
Parameter-Trust Update accumulates from gradient evidence, feature trust and label
trust during training. It is the part of the framework that no confidence score,
uncertainty estimate or input-space detector expresses. Read under a fully trusted
input, it localises training-data poisoning at the level of individual classes,
separating the two flipped classes from every clean class by a margin far outside
the clean-class band, distinguishes the label channel from the trigger channel, and
provides a training-quality tripwire that flags label corruption at every non-zero
flip rate without any test data, robust to the dead-unit confound and to the
threshold convention. The composed per-sample score of \cref{sec:composed} combines
the two routes with the model's confidence; its operational reading is to gate on
trust and on the applicability diagnostics, then rank by confidence among trusted
inputs.

The comparative evaluation attributes each result to one route. Where inputs
violate the training marginals, the detection signal comes from the input route,
and propagation acts as the carrier that turns it into an opinion composable with
model trust. Propagation contributes detection signal of its own where the
marginals are silent, under the marginal-preserving translation ($0.781$ against
$0.678$ for the input route alone), although a feature-space nearest-neighbour
detector remains stronger there ($0.801$). The tripwire saturates at a single level
($0.682$ at every flip rate) and does not recover the corruption rate, which the
rate-informed variant and the data-side disagreement baseline do. The theoretical
properties of \cref{sec:ptastheory}, namely the stabilisation of the parameter
trust with the gradients (\cref{thm:convergence}) and the symmetry and invariance
properties of \cref{sec:ptassyminv}, are confirmed in every experiment, and the
evaluation extends to residual convolutional networks at accuracies defensible for
their domains ($99.3\%$ MNIST, $88.7\%$ Fashion-MNIST, $84.5\%$ CIFAR-10) through
the activity-weighted conv \ac{IPTA}.

\paragraph*{Relation to DeepTrust.} DeepTrust~\cite{10.3389/frai.2020.00054}, the
closest prior work (\cref{sec:sl_approaches}), specifies its trust computation at
the output layer and returns one global opinion on the model. \ac{PaTAS-TP} differs
in three ways that the evaluation exercises. The trust computation is defined
neuron by neuron (\cref{def:trustfunc}), so it yields an opinion for every
intermediate activation and every output class; this is what makes the class-level
localisation of poisoning and the \ac{IPTA} possible. The parameter trust is
maintained per parameter and revised at every mini-batch from gradient evidence,
feature trust and label trust (\cref{alg:trustupdate}), rather than being a single
model-level quantity, so the model route can report which classes the training data
affected. And the operators are chosen for the dependence structure of a layered
computation rather than by analogy with addition and multiplication: discounting and
averaging fusion remove the double counting that a cumulative operator would
introduce (\cref{sec:fusion_choice}) and give the framework the invariance
properties of \cref{sec:ptassyminv}.

\paragraph*{Limitations.} Each limitation of the design has a stated remedy.
(i) Each layer applies one round of discounting, so the raw aggregate penalises
depth; the depth-normalized readout of \cref{sec:tp_arch_ovvw} corrects the
geometric component, and a depth-invariant propagation rule remains open.
(ii) The trust functions do not see weight or activation magnitudes, so an input the
network ignores still contributes to the output opinion; the softmax-weighted
aggregation of Experiment~3 and the activity-weighted conv \ac{IPTA} are partial,
influence-aware steps, and fully magnitude-aware trust functions remain open.
(iii) By \cref{thm:infvac}, output disbelief can only originate from input
disbelief, so unreliable parameters manifest as output uncertainty rather than as
an explicit distrust signal; every experiment that feeds a fully trusted input
therefore reports $d = 0$ by construction, and letting parameter distrust
contribute a distrust component to the output is a possible relaxation.
(iv) The conformity opinions detect support violations, not support subsets; the
diagnostics predict this failure, and fusing a covariance- or neighbourhood-based
detector into the input opinion through the same quantification is the natural
\ac{SL} remedy. (v) The gradient-counting rule of \textsc{NodeTrust} fixes the
uncertainty of each parameter opinion by the fan-in and initialises
$T_{n_i \mid \overline{y_\text{batch}}}$ to the vacuous opinion; adaptive
quantification and richer priors are further refinements. Extending the trust
functions to transformer components and normalization layers, and scaling the
evaluation to larger training pipelines, are taken up in
