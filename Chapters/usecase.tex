...
\caption{Selective prediction comparison: \ac{PaTAS} belief-over-uncertainty filter ($b > u$) versus NN softmax score thresholds ($\tau = 0.5$ and $\tau = 0.8$). Coverage is the fraction of test samples retained by each filter; accuracy on covered is the accuracy over those retained samples. Values are mean $\pm$ std over 5 independent runs.}
\label{tab:selective-prediction-merged}
\begin{tabular}{lcccccc}
\toprule
 & \multicolumn{2}{c}{PaTAS ($b > u$)} & \multicolumn{2}{c}{NN softmax $\tau = 0.5$} & \multicolumn{2}{c}{NN softmax $\tau = 0.8$} \\
