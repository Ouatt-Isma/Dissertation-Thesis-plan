...
This formulation allows uncertainty arising from missing, incomplete, or ambiguous evidence to be modeled explicitly, instead of being conflated with aleatoric randomness. 
 ...
 An alternative framework for representing uncertainty under incomplete information is Belief function theory introduced by Dempster and Shafer. Rather than assigning probability mass exclusively to single events, belief functions distribute mass over sets of hypotheses, allowing part of the belief to remain uncommitted and explicitly representing ignorance. One benefit of belief function theory is that it allows rule of combination which defines how independent sources of evidence may be fused. However, Baron shows that belief functions can be interpreted as a \emph{restricted special case} of second-order probability models, corresponding to strong structural constraints on the admissible forms of the distribution \( Q(P) \)~\cite{Baron1987}. As a result, belief functions sacrifice expressive generality in exchange for a simplified representation of uncertainty.

This relationship is particularly relevant for uncertainty-aware trust modeling. Second-order probability provides a principled probabilistic foundation for reasoning about uncertainty in probability estimates, while belief-function-based frameworks offer practical tools for aggregating evidence and representing ignorance. These ideas directly inform later developments such as Subjective Logic
% , which combines second-order uncertainty representation with algebraic operators for trust discounting and fusion
.
...
A central component of \ac{DST} is Dempster’s rule of combination, which provides a mechanism for fusing evidence from multiple independent sources~\cite{SHAFER201626}. 
...
These limitations motivate extensions of \ac{DST} that explicitly incorporate trust modeling, source dependency and trust transitivity. Subjective Logic~\cite{josang2016subjective} builds upon \ac{DST} by introducing subjective opinions, trust discounting, and explicit uncertainty mass, thereby enabling trust reasoning and evidence fusion in distributed and multi-agent environments.

\todo[inline]{Reviewed until here}

\section{Subjective Logic and Trust}
\begin{itemize}
    \item Deep Learning Based Scalable Inference of Uncertain Opinions
    \item Using Subjective Logic to Estimate Uncertainty in Multi-Armed Bandit Problems 
    \item Learn to Accumulate Evidence from All Training Samples
\end{itemize}
\todo[inline]{Discuss all work on SL: SLencondings and all others}
...
\begin{align}
    ...
    \beta = r_x + a_xW = 0 + 0.5\times2 = 1
\end{align}


For a fix  non-informative prior weight \(W\), knowing the mapping (\cref{eq:map_r_to_alpha}) from beta parameters (\(\alpha\) and \(\beta\)) to positive and negative evidence (\(r_x\) and \(s_x\)). The mapping between a binomial opinion 
...
Like Binomial subjective opinions and Beta distribution, there exists a bijective mapping between multinomial subjective opinions and Dirichlet \acp{PDF}. This mapping generalizes the binomial–Beta correspondence.
...
With non-informative prior weight $W$, the Dirichlet strength parameters are expressed as:
...
The Two-Edge Path discounting operator models trust propagation across exactly two adjacent edges: a \emph{referral trust} edge from agent $A$ to agent $B$, and a \emph{functional trust} (or \emph{functional belief}) edge from agent $B$ to a variable $X$. This situation is illustrated in Figure~\ref{fig:tdis}. The resulting discounted opinion represents the belief of $A$ about $X$, derived via its referral trust in $B$.
...
This situation can be naturally modeled as a Subjective Trust Network. Vehicle $A$ holds referral trust opinions $\omega^A_B$ and $\omega^A_C$ toward vehicles $B$ and $C$, respectively, reflecting how much $A$ trusts them to provide reliable information. Vehicles $B$ and $C$ each also hold referral trust opinions $\omega^B_{D}$ and $\omega^C_{D}$ about how much they trust \(D\) for providing reliable information about its velocity\(v_D\). Finally vehichle \(D\) has a functional trust on how accurate its reported velocity \(v_D\) is.
...
This example illustrates how \acp{STN} enable trust computation in distributed systems. Trust discounting captures the attenuation of trust across intermediaries, while belief fusion aggregates evidence from multiple sources. Moreover we can see that \(\omega^A_B\) and \(\omega^B_{D}\) are both referral trust opinion, therefore existing trust discounting operators cannot be used in this case. Improving trust discounting operators is then needed. \ac{SL} mechanism is essential for enabling cooperative decision-making. This motivate the need for structurally sound trust reasoning frameworks, as discussed in the following section.
...
\right|
\end{equation}
where \(\mathcal{P}\) denotes the set of \acp{PPS} in \(G\).
\end{definition}

Figure~\ref{fig:ppsv} illustrates an example of \ac{DSPG} and its PPS structure. 
...
\begin{figure}[ht]
        ...
        \label{fig:ppsv}
\end{figure}
Based on these definitions, J{\o}sang proposes an algorithm for computing trust over \acp{DSPG} by iteratively reducing PPSs into single edges (\cref{fig:algsynth1}). At each reduction step, the Nesting Level is updated accordingly, ensuring that the order of discounting and fusion operations remains consistent with the graph structure.
...
When the \ac{STN} does not satisfy the \ac{DSPG} property, it must be transformed into a \ac{DSPG} before being analyzed (\cref{fig:dspg_synth_analyse}). 
...
\begin{enumerate}
    ...
    \item The source and target nodes must have equal nesting levels.
...
\end{enumerate}
These conditions ensure that the addition of a branch does not introduce structural inconsistencies during the reduction process. However, they can be overly restrictive. In particular, requiring equality of nesting levels prevents the addition of edges from a PNPS source to one of its intermediate nodes, or from an intermediate node to its target, even in cases where such additions would not violate the \ac{DSPG} properties.

The \ac{NNL} is defined as follows.
\begin{definition}[Node Nesting Level]
Let $v$ be a node in the graph. The nesting level of $v$, denoted by $\mathrm{NNL}(v)$, is defined as follows:
\[
\mathrm{NNL}_G(v) =
\begin{cases}
\displaystyle \max_{ v \in \mathcal{P}} \mathrm{PNL}(\mathcal{P}), & \text{if } v \text{ is an intermediate node of at least one PNPS}, \\
0, & \text{otherwise},
\end{cases}
\]
where $\mathcal{P}$ denotes the set of PPSs of the graph \(G\).

The nesting level of a PPS $\mathcal{P}$ is defined as
\[
\mathrm{PNL}(\mathcal{P}) = \min_{e \in \mathcal{P}} \mathrm{NL}(e),
\]

In particular, since the source and sink nodes of the entire graph are not intermediate nodes of any PNPS, it follows that
\[
...
\[
\mathbf{o}(x) = f_\theta(x),
\]
where \( f_\theta(x) \) denotes the deterministic computation performed by the network with fixed parameters \( \theta \). The last layer and its activation are chosen depending on the task. In classification settings, They are selected so that the output components lie in the interval \( [0,1] \) and can be interpreted as class scores or probability-like values.
...
As a result, they may produce overconfident predictions for ambiguous inputs, noisy samples, or data drawn from regions outside the training distribution.
...
In addition to Bayesian and sampling-based approaches, several deterministic methods aim to estimate uncertainty using a single forward pass. Softmax confidence scores and predictive entropy are commonly used as simple uncertainty proxies, but they are known to be poorly calibrated and unreliable under distribution shift~\cite{guo2017calibrationmodernneuralnetworks}.
...
\subsection{Subjective Logic Approaches to Trust and Uncertainty in \ac{NN}}
...
\todo[inline]{Make the limitations clear}
\todo[inline]{Check other dissertation and take idea from there}

\todo[inline]{read everything again from scratch}
\todo[inline]{Submit it to Houda}