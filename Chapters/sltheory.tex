...
\chapter{Optimization of \acs{DSPG} Synthesis and \aclu{TNA-SL} [31/03]}
\label{chap:tnasl}
...
\ac{DSPG} synthesis approach proposed in the original work of J{\o}sang~\cite{10.5555/1151699.1151710} incrementally adds branches while verifying structural criteria. This method relies on overly restrictive conditions and often removes edges considered structurally unsafe, even though adding these edges would not violate \ac{DSPG} compliance. This issue introduces two major limitations.
\begin{enumerate}
...
To address the limitations of \ac{DSPG} synthesis and analysis procedures identified in the introduction, we refine the notion of \ac{PPS} originally used in \ac{DSPG} analysis. While \ac{PPS} structures are sufficient for defining the reduction order in classical \ac{DSPG} analysis, they are not strong enough to guarantee structural isolation during analysis and \ac{NL} updates.

For example, consider the graph shown in \cref{fig:ppsv,fig:pnps}. If the \ac{PPS} between nodes \(A\) and \(C\) is reduced into a single edge \(A\to C\), the \ac{NL} of this newly created edge becomes \(0\), while the \acp{NL} of the remaining edges decrease by one. This global update illustrates that reductions based on \ac{PPS} structures do not preserve local structural isolation during the reduction process, since the transformation affects the \aclp{NL} of edges outside the reduced subnetwork.

To overcome this limitation, we introduce a stricter structural construct called the \acf{PNPS}. This refinement plays a central role in enabling both a clearer formalization of \ac{DSPG} structures and stable \ac{NL} computation after \ac{PNPS} reduction into a single edge.
...
\end{definition}
The key difference between \ac{PPS} and \ac{PNPS} lies in the non-intersection requirement. This distinction is illustrated in \cref{fig:pnps}. For instance, although $(A,G)$ qualifies as a \ac{PPS}, it is not a \ac{PNPS} because we cannot find two non-intersecting paths between $A$ and $G$. On the other hand, if we add a path from $A$ to $G$ via a new node $H$, then $(A,G)$ becomes a \ac{PNPS}. 
...
The introduction of \ac{PNPS} structures enables a simpler characterization of structural violations in a graph. In particular, the following proposition provides a structural way for detecting graphs that do not satisfy the \ac{DSPG} property.
...
This completes the structural synthesis component of the framework. In the next section, we turn to optimized \ac{DSPG} analysis and \acp{NL} computation during reduction.
...
\begin{enumerate}
    \item Series reduction, which reduces two consecutive edges incident to a node of degree two.
    \item Parallel reduction, which reduces parallel edges into a single edge.
\end{enumerate}

However, in the \ac{PPS}-based framework these reductions are not structurally isolated. Replacing a \ac{PPS} with a single edge may alter the structure of other \acp{PPS} in the graph. For example, in \cref{fig:pnps}, reducing the \ac{PPS} \(A \to C\) modifies the connectivity of the graph and the subnetwork \(A \to G\) is no longer a \ac{PPS}. As a consequence, reductions can propagate structural changes across the graph, making the evolution of \acp{NL} difficult to track precisely.

In this section, we show that replacing \ac{PPS} with \ac{PNPS} yields a structurally localized reduction process.  We formalize the corresponding reduction procedure and analyze the resulting \aclu{NL} updates.
...
The analysis algorithm proceeds by iteratively selecting a \ac{PNPS} with maximum \ac{NL} and reduces it. The key advantage of \ac{PNPS} is that structural isolation is guaranteed (\cref{fig:pnps}). Since the parallel paths are node-disjoint, reducing the most nested subnetwork does not create unintended structural interactions. 

The proposed optimized analysis algorithm relies on the fact that reducing a \ac{PNPS} allows updates of \acp{NL} in a controlled manner.
...
Let $G$ be a \ac{DSPG}. Reducing a  \ac{PNPS} with maximum \ac{NL} preserves the nesting structure of all remaining  \acp{PNPS}.
\end{theorem}
...
\end{proof}
This ordering guarantees that reduction progresses from the most deeply nested structures outward, ensuring easy \ac{NL} updates.
...
In \ac{DSPG} Analysis, trust propagation along referral paths plays a central role in deriving consistent opinions between distant nodes. While classical trust discounting operators allow the combination of a referral edge with a functional edge, they do not guarantee consistency when multiple referral edges appear consecutively in a path. In particular, the projected probability of the resulting opinion is not always equal to the product of the projected probabilities along the path, which makes it inconsistent with the Multi Edge Trust Discounting. In clear, if we have a pattern in a graph like the one depicted in \cref{fig:rs2} 
\begin{align} \label{eq:TE}
    \omega^{A_1}_{A_n} &= \omega^{[A_1;\ldots;A_{n-2}]}_{A_{n-1}}\otimes_{_{ME}} \omega^{A_{n-1}}_{A_{n}} \\
    &=(\omega^{A_1}_{A_2} \otimes_{_{TE}}( \dots \otimes_{_{TE}} (\omega^{A_{n-2}}_{A_{n-1}} \otimes_{_{TE}} \omega^{A_{n-1}}_{A_{n}}) \notag \\ 
    &\neq((\omega^{A_1}_{A_2} \otimes_{_{TE}} \dots )\otimes_{_{TE}} \omega^{A_{n-2}}_{A_{n-1})} \otimes_{_{TE}} \omega^{A_{n-1}}_{A_{n}} \, \text{non associativity of } \otimes_{_{TE}} \notag
\end{align}
...
Associativity ensures that trust propagation over referral chains in optimized \acp{DSPG} is independent of grouping, which is essential for generalization of the operator.
...
\subsection{Evaluation of \acs{DSPG} Synthesis + Analysis Computational Time}
\todo[inline]{Evaluate the Time: Make \ac{DSPG} Synthesis+analysis using previous criteria and method vs our criteria. Record the average time as the size of the ACG increases}
\todo[inline]{Language to use. Probably python. Code for our Method is already available in Go, but we will have to code the Original one}
...
To further analyze operator behavior, we consider the generic referral path illustrated in \cref{fig:rs2}. The path contains only referral edges, each assigned the identical opinion:
\[
(0.5, 0.4, 0.1, 0.8)
\]
...
% The proposed Referral-Edge Path operator therefore satisfies two essential properties:
% \begin{enumerate}
%     \item Associativity over referral chains.
%     \item Preservation of projected probability multiplicativity.
% \end{enumerate}

% These properties make it probabilistically consistent for modeling transitive trust in Subjective Trust Networks.
