...
\todo[inline]{Make clear that aggregation is the generic word for fusion. in case we need to combine in a way that is not already represented by existing fusion operators}
...
The objective of this chapter is to abstract and generalize this evidence-based reasoning process so that it can be instantiated across different \ac{AI} contexts. While the technical mechanisms used to compute trust may vary across systems, the underlying reasoning process remains largely invariant. The methodology introduced here therefore provides a unified framework for structuring trust assessment problems and deriving trust evaluations from observable evidence.

In the remainder of this dissertation, this general methodology will be instantiated in several distinct settings:

\begin{itemize}
    
    \item the evaluation of training dataset trustworthiness (\cref{chap:dataset-trust}),
    \item the propagation of trust through neural network architectures using the PaTAS framework (\cref{chap:patas}),
    \item the trust assessment of \aclu{NN} prediction calibration (\cref{chap:bb}).
\end{itemize}
...
The terminology follows the principles of the \ac{TAF}, which adopts \acf{SL} as the formal reasoning framework for representing and assessment trust. The notation introduced here is therefore consistent with \ac{SL} representations.
...
where each $T_i$ represents independent simple trust proposition.
...
where $t$ denotes the trust mass (belief of the binomial opinion), $d$ denotes the distrust mass (disbelief of the binomial opinion), and $u$ represents uncertainty. This representation allows the trust assessment to capture not only whether the trustor should trust the proposition, but also the degree of confidence associated with that assessment.
...
Let $\mathcal{T}$ denote a trust proposition. In some cases, $\mathcal{T}$ is not directly measurable. Instead, it must be decomposed into a set of simple propositions that correspond to individually assessable properties:
\[
...
This structured pipeline ensures that trust assessment remains transparent, reproducible, and adaptable across domains. In subsequent chapters, this generic process will be instantiated for dataset trustworthiness, \ac{NN} trust propagation, and \ac{NN} calibration assessment.
