...
Moreover, the internal decision-making processes of \acp{NN} are often opaque, limiting the user’s ability to monitor and validate individual predictions. As a result, reliance on such systems closely mirrors the trust relationships described in both organizational and formal \ac{ICT} settings: it involves accepting risk based on expectations about the system’s competence, reliability, and underlying assumptions. This dual nature of trust, as both vulnerability acceptance and property-based belief, highlights the need for formal, interpretable, and evidence-based approaches to trust assessment in \ac{NN} systems.\todo{make clear that these properties also transfer to technical trust, like an automated vehicle relying on the outputs of a NN in its driving decisions.}

\marginpar{Limits of Current Metrics}
Despite this conceptual alignment, most existing approaches for evaluating \acp{NN} focus primarily on predictive performance metrics such as accuracy, precision, or recall. While these metrics are essential, they fail to capture whether a model is trustworthy in the sense required for high-stakes decision-making.\todo{your focus here is largely on predictive AI. At some point, it might be worth also having a short discussion about GenAI and how trust in it can be considered and if or if not it's in or out of scope. This could then be brought up again in outlook and future work.} A highly accurate model may still be unsafe if it is poorly calibrated, biased, or trained on unreliable data. Conversely, a model with lower predictive accuracy may be more trustworthy if it appropriately represents uncertainty and avoids overconfident predictions in unfamiliar or out-of-distribution scenarios.

Furthermore, trust in \acp{NN} is not solely a property of the model architecture or its learned parameters. It is inherently multi-layered and depends on the quality of the training data, the reliability of annotations, the behavior of the learning process, and the way uncertainty propagates\todo{I think the term propagation requires more explanation. It isn't that trust just straightforwardly propagates through the NN. It's that trustworthiness of one component depends on another component and these dependencies create transitive dependencies that one can calculate.} from inputs to outputs. Dataset biases, labeling inconsistencies, and hidden correlations can undermine trust long before inference is performed. Without explicit mechanisms to assess and propagate trust across these components, users are left with opaque confidence scores that provide limited support for risk-aware decision-making.

These limitations motivate the need for a formal and interpretable framework for trust in \acp{NN}. Such a framework should explicitly represent uncertainty and support principled reasoning about how trust evolves as new information becomes available\todo{ and it should allow to reason over trustworthiness from the perspective of a specific subject / entity in a complex system-of-systems.}. It should also align with established theories of trust and risk, while remaining operational within modern \ac{ML} pipelines. This thesis addresses these requirements by grounding trust assessment in \acf{SL} and by developing mechanisms to quantify, propagate, and aggregate trust across datasets, network structures, and model outputs.


\subsection{Challenges in Quantifying Trust and Uncertainty in Neural Networks}
\label{sec:quantification_challenges}
Although the need for trust in \ac{NN} based systems is increasingly acknowledged, the quantification of trust and uncertainty remains an open and challenging problem. Unlike classical performance metrics, trust is not directly observable and cannot be measured through a single empirical quantity\todo{making such strong statements should typically be backed up by references fostering that claim. Otherwise, a reader / reviewer might disagree and ask, why it can't be represented just by, say, a probability.}. Instead, it emerges from a combination of evidence, uncertainty, and risk\todo{These require more precise definitions later in the following chapters.}, all of which must be represented in an interpretable manner.
...
This thesis aims to develop a formal, interpretable, and operational framework for assessing trust in \ac{NN} pipelines\todo{be careful with the wording. A ML pipeline has a defined meaning that rather focusses on the process of training, validation, evaluation, and deployment, rather than on the data flow. This might be confusing and maybe the term pipeline should be replaced with NN models?}, both during training and inference. In particular, such framework should model how trust propagates from the training data to the learned model parameters, and from input observations to output predictions\todo{ this clearly states that your focus is on predictive AI. You should explain at one point that GenAI is not your focus (and why and if your approach could transfer to GenAI).} through the model. While traditional evaluation approaches primarily rely on predictive performance, they fail to capture essential aspects of trust such as uncertainty, incomplete knowledge, and the reliability of information sources. As \ac{NN} systems become increasingly complex and are deployed in safety-critical and open environments, these limitations become more pronounced.

To address this gap, this work investigates how trust can be modeled as an evidence-based quantity and systematically integrated across the different stages of the \ac{ML} system pipeline. In particular, the thesis explores how \ac{SL} can be adapted and used to represent, quantify, and combine trust\todo{At one point in the thesis, you should discuss the distinction between trust and trustworthiness and how you use these terms. Recall the discussions we had in CONNECT on this.} in \ac{NN} systems. This includes assessing the trustworthiness of training datasets, modeling the propagation of trust through structured dependencies, and evaluating black-box models through observable signals such as calibration behavior. this clearly states that your focus is on predictive AI. You should explain at one point that GenAI is not your focus (and why and if your approach could transfer to GenAI).
...
Indeed, many \ac{ML} pipelines, and especially \acp{NN}, can be interpreted as large computational graphs in which information flows through multiple interconnected components, including data sources, features, parameters\todo{Is data really flowing through a parameter? And I miss terms like neurons or perceptron here?}, and intermediate representations.
...
These limitations restrict the applicability of \ac{SL} in complex \ac{ML} systems where trust must be evaluated across multiple interconnected components of the data and learning pipeline. In such settings, inaccuracies or inconsistencies in intermediate trust computations can propagate and amplify throughout the system, potentially leading to misleading or unreliable trust assessments at the output level. Therefore, there is a need to define methods that reduce computational overhead, improve the robustness of trust quantification, and ensure the consistency of trust inference when applied to such settings.\todo{ How would robustness be addressed? From your current text, you basically raise that the order of evaluation must be consistent and that performance must be ok for practical applications. Robustness is not really addressed in detail (above you say something about 'information might get lost', but this is not sufficiently clear).}
...
Building upon the representation of trust and the quantification of dataset trustworthiness, a central challenge lies in understanding how trust propagates throughout the learning and inference processes. In \ac{ML} systems, trust is not confined to isolated components, but instead evolves\todo{Again, wording of trust propagation and evolution should be checked. Maybe some better terminology can be found.} across multiple stages, from the training data to the learned model parameters, and from input observations to output predictions.
...
\todo{Make clear that we didn't introduce but it was a collaboration}
...
While the previous contributions assume access to the internal structure of the model, many real-world systems operate under limited observability. This motivates the need for complementary approaches that rely solely on observable data, such as model outputs. This approach enables trust evaluation without access to the internal model parameters or architecture, thereby extending the applicability of the framework to practical deployment scenarios. This contribution addresses \textbf{RQ4} by enabling trust assessment of calibration of a \ac{ML} model. \todo{this contribution is less specific than the others and somewhat blurry in its description}
