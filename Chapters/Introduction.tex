...
The objective is therefore to provide a unified methodology that enables practical computation, interpretation, and use in real-world \ac{NN} systems. Based on these objectives and the challenges identified earlier, we formulate the following research questions.
...
In such settings, performing trust inference using \ac{SL} over large and complex graph structures can lead to significant computational overhead, as the number of operations required to propagate and aggregate opinions grows rapidly with the size and architecture of the system. 
...
This creates a fundamental gap: although data is the foundation of learning, there is no framework that both quantifies dataset trustworthiness in a 
formally grounded way and preserves this information so that it can be 
carried forward into subsequent trust assessment during training and inference. In particular, datasets are rarely modeled as structured sources of evidence that can be integrated into a broader trust reasoning framework.
...
This thesis proposes a unified and operational framework for trust assessment in \ac{ML} systems, with a particular focus on \aclu{NN} pipelines. 
The contributions are structured into four main components,  illustrated in \cref{fig:contributions}.
